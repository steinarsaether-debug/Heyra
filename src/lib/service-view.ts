...
export async function getNearbyServicesForListing(
  ...
  const rows = await prisma.$queryRaw<NearbyServiceResult[]>`
    ...
    FROM (
      SELECT
        ...
        CASE
          ...
          ELSE NULL
        END AS "distanceKm",
        CASE WHEN s."municipality" = p."municipality" THEN 0 ELSE 1 END AS "municipalityPriority",
        s."updatedAt"
      FROM "Listing" l
    ...
    ORDER BY
      nearby."municipalityPriority",
      CASE WHEN nearby."distanceKm" IS NULL THEN 999999 ELSE nearby."distanceKm" END,
      nearby."updatedAt" DESC
    LIMIT ${limit}
  `;
